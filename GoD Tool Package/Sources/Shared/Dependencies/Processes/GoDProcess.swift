...
class GoDProcess {
	...
	func readVirtualMemory(at offset: UInt, length: UInt, relativeToRegion region: VMRegionInfo? = nil) -> GoDData? {
	...
	@discardableResult
	func writeVirtualMemory(at offset: UInt, data: GoDData, relativeToRegion region: VMRegionInfo? = nil) -> Bool {
		guard let task = self.task, let baseAddress = self.baseAddress else {
			if XGSettings.current.verbose {
...
class GoDProcess {
	...
	func readVirtualMemory(at offset: UInt, length: UInt, relativeToRegion region: VMRegionInfo? = nil) -> GoDData? {
		...
		let data = GoDData(byteStream: buffer)
		return data
	}
